...
$vidSQL = "SELECT alecvid, received_date FROM alecsitetest.submitted WHERE alecvid IN (SELECT alecvid FROM alecsitetest.verified WHERE (verified.alecvid NOT IN (SELECT alecvid FROM alecsitetest.approved))) ORDER BY received_date ASC;";
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $alecvid = $_POST["alecvidapp"];

    $existsSQL = "SELECT * FROM alecsitetest.verified WHERE (verified.alecvid NOT IN (SELECT alecvid FROM alecsitetest.approved) AND (verified.alecvid = '$alecvid'));";
    $exists = $db->query($existsSQL);
    $exists = $exists->fetch_assoc();
    if (!is_null($exists)) {
        $_SESSION['alecvidapp'] = $alecvid;
...
<div class = valverapp>
        ...
        <script>
            ...
            const createTable = () => {
                ...
                for (let i = 0; i < vidbody.length; i++) {
                    let vidrow = JSON.stringify(vidbody[i]);

                    let vidjson = JSON.parse(vidrow);
    ...
    <div class = "v">
        <form method = "post">
            <div>
                <h4>ALECVID</h4>
                <input type="text" id="ALECVIDVAL" name="alecvidapp">
                <input type = "submit">
            </div>
        </form>
    </div>
